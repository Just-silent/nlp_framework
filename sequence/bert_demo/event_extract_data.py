...
class SequenceDataLoader(CommonDataLoader):
    ...
    def __build_field(self):
        self.TEXT = Field(sequential=True, use_vocab=True, tokenize=tokenizer, include_lengths=True, batch_first=self._config.data.batch_first, pad_token='[PAD]', unk_token='[UNK]')
        self.TAG = Field(sequential=True, use_vocab=True, tokenize=tokenizer, is_target=True, batch_first=self._config.data.batch_first, pad_token='[PAD]', unk_token='[UNK]')
        self._fields = [
