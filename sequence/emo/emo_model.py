...
class EmoModel(CommonModel):

    def __init__(self, seq_config_file):
        super(EmoModel, self).__init__(seq_config_file)
    ...
    def forward(self, dict_inputs: dict) -> dict:
        ...
        encoded, (hidden, _) = self.lstm_encoder(text_embedding)
        hidden = hidden.view(text_embedding.size(0), -1)
        emissions = torch.softmax(self.linner(hidden), dim=-1)
        dict_outputs['emissions'] = emissions
        dict_outputs['target_sequence'] = tag
        if self.training:
            dict_outputs['loss_batch'] = F.cross_entropy(emissions, tag)
        else:
